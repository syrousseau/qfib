...
///
/// \brief computeRatio Compute the ratio of the input fiber (only one)
/// \param fibers Fiber from which to compute the ratio
/// \return Ratio of the unique fiber
///
...
float computeRatioVerif(const std::vector<std::vector<Vector3f> > & fibers, bool verbose)
{
    std::vector<float> mindot(omp_get_max_threads(), 2.0f);
    std::vector<float> maxStep(omp_get_max_threads(), 0.0f);
    std::vector<float> minStep(omp_get_max_threads(), std::numeric_limits<float>::max());
...
float computeRatio(const std::vector<std::vector<Vector3f> > & fibers, bool verbose)
{
    std::vector<float> mindot(omp_get_max_threads(), 2.0f);
#pragma omp parallel for
    for(int f = 0; f < fibers.size(); ++f)
...
template<typename T>
void compressFiber(const std::vector<Vector3f> & fiber, const float ratio, CompressedFiber<T> & cfiber, float & maxError, float & meanError)
{
    assert(fiber.size() > 1);
