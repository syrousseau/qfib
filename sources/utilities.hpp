...
void meanMaxError(const std::vector< std::vector<Vector3f> > & fibersA, const std::vector< std::vector<Vector3f> > & fibersB, float & meanError, float & maxError)
{
    meanError = 0.f;
    maxError = 0.f;
    unsigned nbPoints = 0;
...
void error(const std::vector<std::vector<Vector3f> > & fibersA, const std::vector< std::vector<Vector3f> > & fibersB,
           vector<float>& minError, vector<float>& meanError, vector<float>& maxError,
           float & minEndPointError, float & meanEndPointError, float & maxEndPointError, unsigned steps = 10,
           float minValue = 35, float maxValue = 260)
{
    ...
    vector<unsigned> nbPoints(nbSteps, 0);

    unsigned nbEndPoints = 0;
    assert(fibersA.size() == fibersB.size());
    for(unsigned f = 0; f < fibersA.size(); ++f)
    {
        ...
        nbPoints[tablePos] += fibersA[f].size();
        nbEndPoints++;
        assert(fibersA[f].size() == fibersB[f].size());
        for(unsigned p = 0; p < fibersA[f].size()-1; ++p)
        {
            float err = (fibersA[f][p] - fibersB[f][p]).norm();
            meanError[tablePos]  += err;
