...
void compareFA(std::vector<std::vector<Vector3f>> const & fibers1, std::vector<std::vector<Vector3f>> const & fibers2, const string & faFilename)
{
    ...
    std::vector<float> ecartMoyenTab(omp_get_max_threads(), 0);
    std::vector<uint64_t> nbPointsTab(omp_get_max_threads(), 0);
    std::vector<float> ecartMaxTab(omp_get_max_threads(), 0);
    std::vector<float> averageFiber1Error(omp_get_max_threads(), 0);
    std::vector<float> averageFiber2Error(omp_get_max_threads(), 0);
#pragma omp parallel for
    ...
    for (unsigned i=1; i<omp_get_max_threads(); i++)
    {
    ...
    ecartMoyen /= nbPoints;
    averageFiber1 /= (float)omp_get_max_threads();
    averageFiber2 /= (float)omp_get_max_threads();
